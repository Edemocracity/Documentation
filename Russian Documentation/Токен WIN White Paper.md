...
**5,000,000WIN: Предложено для Period O** По цене **1000WIN = 0.35ETH**. Это небольшой период распределения токенов для раних инвесторов. Период закончится когда все предложеные для периода токена будут распределены или вместе с началом Period 1 .*Все не проданные токены, будут уничтожены в конце периода.*

**15,000,000WIN: Предложено для Period 1** По цене **1000WIN = 0.5ETH**. **Период начнется:** 3.10.2017; 14:00UTC. Period 1 закончится когда все предложенные для периода токены будут распределены или по истечению 161 часа, после чего новый период начнется без замедлений. *Все не проданные токены, будут уничтожены в конце периода.*
...
Переведите ETH на смарт-контракт распределения  токенов https://etherscan.io/address/0xaa3e80a42e7bac1072bee7ee5ac72123969b8400, для этого вы можете использовать ERC20 совмещеный Ethereum Wallet: Ethereum Foundation Wallet, Mist, Meta Mask, Parity, imToken(iPhone/Android)

**ОТПРАВЛЯЙТЕ ETH ТОЛЬКО НА ЭТОТ АДРЕС:>>** ``0xaA3e80a42E7Bac1072BEe7Ee5Ac72123969b8400``
...
Сейчас если вы не знаете, как это сделать спросите в нашем канале Telegram.
